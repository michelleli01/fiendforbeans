...
MYSQL_USER = "root"
MYSQL_USER_PASSWORD = "beijing930916"
...
def index_search(query, index, idf, doc_norms, tokenizer, score_func=accumulate_dot_scores):
    ...
    for doc_id, doc_score in doc_scores.items():
        cossim_val = doc_score / (bean_doc_norms[doc_id] *  q_norm)
...
output = index_search(query, inv_idx, idf, tokenize, bean_doc_norms) #score, doc id 
...
for score, bean_id in output:
    ...
    rec_beans.append((bean_info['name'], score))
print(rec_beans) 
